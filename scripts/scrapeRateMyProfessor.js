 ...
 * Fetches real UW professor data
 */
...
async function scrapeAllSchools() {
  ...
  for (const [schoolName, schoolId] of Object.entries(schoolIds)) {
    console.log(`  Scraping ${schoolName} (ID: ${schoolId})...`);
    try {
...
async function main() {
  try {
    ...
    console.log(`\n✅ Total professors scraped: ${professors.length}`);
    
    if (professors.length > 0) {
      console.log('\nSample professors:');
      professors.slice(0, 5).forEach(p => {
        console.log(`  - ${p.firstName} ${p.lastName} (${p.school}): ${p.avgRating}⭐ (${p.numRatings} ratings)`);
      });
    }
  } catch (e) {
